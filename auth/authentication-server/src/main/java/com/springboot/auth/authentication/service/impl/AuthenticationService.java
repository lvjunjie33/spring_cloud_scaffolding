...
//    	Set<RequestMatcher> set=resourceConfigAttributes.keySet();
    	ConfigAttribute configAttribute=null;
//    	for (RequestMatcher str : set) {  
//    	      if(str.matches(authRequest)){
//    	    	  c=resourceConfigAttributes.get(str);
//    	      }  
//    	}
//    	System.out.println(c.getAttribute());
//    	return c;
    	
    	for (Map.Entry<RequestMatcher, ConfigAttribute> entry : this.resourceConfigAttributes.entrySet()) {
    		 if(entry.getKey().matches(authRequest)){
    			 configAttribute=resourceConfigAttributes.get(entry.getKey());
    		 }
        }
    	if(configAttribute==null) {
    		configAttribute=new SecurityConfig(NONEXISTENT_URL); 
    	}
...
//                .collect(Collectors.toMap(RequestMatcher, ConfigAttribute));
//                .peek(urlConfigAttribute -> {
//                	log.debug("url在资源池中配置：{}", urlConfigAttribute.getAttribute());
//                 })
//                .findFirst()
