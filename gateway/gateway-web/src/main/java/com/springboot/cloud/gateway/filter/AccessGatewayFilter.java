...
import com.springboot.cloud.gateway.auth.service.IAuthService;
import com.springboot.cloud.gateway.auth.service.impl.AuthService;
...
public class AccessGatewayFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        //校验token凭证是否有效
        if(authService.checkJwt(authentication, url, method)) {
        	return unauthorized(exchange);
        }
        Result result=authService.hasPermission(authentication, url, method);
        //系统异常
        if(result.isFail()) {
        	return inServerErro(exchange);
        }
     ...
     * @param
     */
    private Mono<Void> inServerErro(ServerWebExchange serverWebExchange) {
        serverWebExchange.getResponse().setStatusCode(HttpStatus.INTERNAL_SERVER_ERROR);
        DataBuffer buffer = serverWebExchange.getResponse()
                .bufferFactory().wrap(HttpStatus.INTERNAL_SERVER_ERROR.getReasonPhrase().getBytes());
        return serverWebExchange.getResponse().writeWith(Flux.just(buffer));
    }
}
